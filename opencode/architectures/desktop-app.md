...
具体的なフレームワークが確定したら、`.opencode/instructions/stack-setup.md` の Step 3.5 ブロックB（必須ファイルの確認・記録）が
...
フレームワーク固有の必須ファイルは `.opencode/instructions/stack-setup.md` の Step 3.5 ブロックB（必須ファイルの確認・記録）が自律検索して記録する。
