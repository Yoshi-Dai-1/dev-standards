...
- **Pulumi**：クラウドリソースをプログラミング言語（TypeScript / Python / Go / C#）で定義する。
  Terraform との最大の違いは「ループ・条件分岐・関数などのコード構造を直接使える」点。
   ディレクトリ構成はブロックC（フレームワーク固有設計の深掘り）が、必須ファイルはブロックB（必須ファイルの確認・記録）が自律検索して調達する。
...
- `.opencode/standards/principles/resilience.md`（バックアップ・障害設計）
- `.opencode/standards/principles/production-readiness.md`（リリース前チェックリスト）
- `.opencode/standards/principles/commercial-operations.md`（コスト管理・SLA設計）
