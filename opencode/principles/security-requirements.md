...
AIがプロジェクト定義を読んでセキュリティ対応レベル（Lv.1〜4）を
人間に聞かずに自律判断するための判断基準。
あわせて対応レベル別・言語別の実装ガイドラインを提供する。

- `.opencode/standards/principles/security-implementation.md` が「どう実装するか（How）」
- このファイルは「何が必要かをどう判断するか（判断基準）」+「対応レベル別・言語別の実装ガイド」
...
### タイミング1：`docs/project-definition.md` が作成・更新されたとき
...
- Web全般 → 使用フレームワーク（`ARCHITECTURE.md`に記録）の公式セキュリティガイドを検索する
  （検索クエリ：[使用フレームワーク名] official security documentation [現在年]）
- 詳細な言語別チェックリストは `.opencode/instructions/security.md` を参照
...
著作権・ライセンス → OSSライブラリを使用するすべてのプロジェクトに適用
  ...
  → ライセンス確認は `.opencode/instructions/stack-setup.md` の Step 3.5 ブロックE（ライセンス確認とプライバシーポリシー要否の判定）が自律実行する
...
| 参照タイミング | 参照元 |
|--------------|-------|
| `docs/project-definition.md` 作成後 | `ARCHITECTURE.md` の Step 3 に組み込まれている |
| 実装依頼を受けたとき | `.opencode/agents/security-designer.md` |
| スプリント開始前 | `AGENTS.md` の Subagents セクション |
| 月次診断時 | `.opencode/skills/live-operation/SKILL.md` の Monthly Checklist |
