...
**注意** 情報の鮮度に限界がある可能性があります。実装前にご確認ください。
```
...
2. 制約内で最も安全な代替案を提示する
3. 「推奨実装からの乖離と理由」を`decisions/`に記録することを提案する
...
5. 発見内容を`decisions/`に記録することを提案する
```
...
4. @security-auditor を呼び出して確認する

5. 判断した内容を`decisions/`に記録することを提案する
```
...
`.opencode/standards/principles/security-requirements.md` の対応レベルが Lv.3以上の場合、
...
設定手順がCIツールの公式ドキュメントで見つからない場合：
  ...
  3. 見つからない場合は「AIの学習データに基づく設定」と明示し、
     人間の確認を得てから設定する
   4. 設定した内容を`decisions/`に記録することを提案する
