# 通信設計ルール（Network Resilience Rules）

このルールはコードファイル・設計ファイル編集時に Plugin が注入する。
...
チェック1：タイムアウトが設定されているか
  ...
  → 実装を止めて通知する:
    「このコードに接続タイムアウトと読み取りタイムアウトが設定されていません。
     `.opencode/standards/principles/network-resilience.md` の原則1に従い実装します。」
...
→ `.opencode/instructions/network-resilience/_ws-graphql.md` を読む
