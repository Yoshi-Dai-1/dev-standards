...
- **一度作成したファイルは上書きしない**
  - 例外：Step 3.5 のブロックBまたはブロックCで決定したランタイムバージョンと、Step 2 で作成したバージョン管理ファイル（`.python-version` / `.nvmrc` / `rust-toolchain.toml` / 等）の内容が一致しない場合、決定したバージョンで上書きする（デフォルト値から確定値への更新のみ許可）
