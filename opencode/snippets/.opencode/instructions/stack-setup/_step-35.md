...
- 実行内容：
  1. `.opencode/standards/principles/naming-conventions.md` の「命名規則の確定手順」セクションに従う（全手順をカバー：Web検索 → 通知 → `ARCHITECTURE.md` 更新）
...
- 実行内容（人間の指示を待たない）：
  ...
  5. `AGENTS.md` の `## Security Boundaries` の末尾に対応事項を追記する
