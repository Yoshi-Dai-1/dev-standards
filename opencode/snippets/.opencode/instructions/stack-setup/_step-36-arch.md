...
3. 必須チェック項目の種類に応じて AGENTS.md の `## Boundaries（禁止事項）` または `## Security Boundaries` の末尾に振り分けて追記する
...
「層のルール」に記入された層定義を使ってリンター設定を生成します。
...
ARCHITECTURE.md の Step 5-B のワークフローをそのまま実行する（追加の指示を待たない）。
