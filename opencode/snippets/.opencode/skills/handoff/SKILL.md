...
1. **現在の状態を保存する**
   `.opencode/handoff-artifact.md` を以下のフォーマットで保存する
...
#### フィードバックループの確認（初回本番リリース時のみ・自律実行）
   ...
   条件B：ARCHITECTURE.md を読み、「開発プロセス」セクションの
   「フィードバック収集：」の値を確認する
   → 「設定済み」と記載されている：このステップを完全にスキップする
   → 「未設定」または該当行が存在しない：以下を実行する

   条件C：docs/project-definition.md を読み、「対象ユーザー」を確認する
   ...
   - 選択した手段を ARCHITECTURE.md の「開発プロセス」セクションの
     ...
     書き換える（このステップが次回以降スキップされるようにするため必須）
...
3. **EOL（サービス終了・アーカイブ）の処理（条件実行）**
   ...
   **Step 1：プロジェクト性質の確認**
   ...
   docs/project-definition.md を読んで以下の2点を確認する：
