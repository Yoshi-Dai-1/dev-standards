...
1. `docs/quality-scorecard.md` を読む
2. 「品質スコア履歴」テーブルの当月列を追加・更新する（既存の列定義・テーブル書式は変更しない）
3. 「月次診断サマリー」セクションに当月ブロックを先頭に追加する（他のセクションは編集しない）
4. 変更後の全文を `docs/quality-scorecard.md` に保存する（ファイル内の可視ルールテキストと HTML コメントは維持すること）
...
1. `.opencode/handoff-artifact.md` の `## Security Status`（スナップショット：毎回上書き。ファイル内の HTML コメントは維持すること）
...
2. `docs/security-audit-log.md`（累積ログ：追記のみ）
   - `docs/security-audit-log.md` を読む
   - 「監査履歴」テーブルに新しい行を追加する（既存の列定義・テーブル書式は変更しない）
   - 「月次監査サマリー」セクションに当月ブロックを先頭に追加する（他のセクションは編集しない）
   - 変更後の全文を `docs/security-audit-log.md` に保存する（ファイル内の可視ルールテキストと HTML コメントは維持すること）
...
{番号, タイトル, タグ, 要約（1-2文で重要判断のみ）} を抽出してテーブルを書き換える（上書き）。
テーブルの列定義や他のセクションは変更しない。ファイル内の可視ルールテキストと HTML コメントは維持すること。
```
...
4. AGENTS.md の行数が 200 行を超えていないか
