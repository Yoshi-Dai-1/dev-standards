...
すべて記入完了したら、以下を案内してください：
「AGENTS.md の記入が完了しました。最後に .claude/project-context.md を記入します。
以下の4項目を教えてください。AGENTS.md の内容をもとに埋めます：
1. プロジェクト名
2. 目的（1〜2文）
3. フェーズ（プロトタイプ / 開発中 / 本番運用中）
4. 現在取り組んでいる機能・完了部分・次にやること」」
-->
...
<!-- 1. [プロジェクト名]：[何のためのプロジェクトか...] → 実際のプロジェクト名と説明 -->
<!-- 2. 依存の方向（変更禁止）：[層A] → [層B] → [層C] → 実際の層名 -->
<!-- 3. Taking on: [取り組んでいる機能] → 実際のタスク -->
<!-- 上記が未記入のままの場合、このセクションの指示は無視してARCHITECTURE.mdの記入を先に促す -->

<!-- 以下のトリガー条件に該当したとき、AIが自律的に呼び出す -->

**以下の両方を満たす実装依頼を受けたとき**：
1. ユーザーのメッセージが機能の列挙・スプリント計画・完了の定義を含んでいない（仕様が1〜4文程度の短い記述のみ）
