...
技術判断が発生したとき、以下の基準に該当すれば `decisions/` への記録を提案する。
実際に書くかは人間が判断する。テンプレート：`dev-standards/snippets/tech-decision.md.template`

**記録すべき判断：**
- 複数の選択肢を比較した結果の選択（ライブラリ・API・アーキテクチャ）
- 後から変えるとコストが高い決断（データモデル・認証方式・外部サービス契約）
- 直感に反する選択をした場合
- ライセンス・法的制約が理由になっている場合
- 以前の方針を変更した場合
