...
同じ情報が複数箇所に存在する状態を禁止する。

```
# 型定義のSSoT
types/api.types.ts    APIレスポンスの型（唯一の定義場所）

# 定数のSSoT
constants/api.constants.ts    APIエンドポイント・タイムアウト値

# 禁止：同じ型が複数ファイルにコピーされている
```
...
// 許可
const SMALL_CAP_THRESHOLD = 300;
if (price > SMALL_CAP_THRESHOLD) { ... }
```
